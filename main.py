...
def handle_list_response(sender, list_reply_id):
    ...
    if step == "department":
        ...
        if selected_department:
            ...
            user_sessions[sender]["department"] = selected_department[0]  # Store department_id
            user_sessions[sender]["department_name"] = selected_department[1]  # Store department name
            ...
            if doctors:
                ...
                send_doctor_list(sender, doctors)
            else:
    ...
    # Handling doctor selection
        ...
        if list_reply_id.isdigit() and 0 < int(list_reply_id) <= len(doctors):
            ...
            user_sessions[sender]["doctor_name"] = selected_doctor[1]  # Store doctor name
...
def send_date_list(sender):
    # Retrieve the selected doctor from the user session
    doctor_id = user_sessions[sender]["doctor"]  

    # Fetch available dates for the selected doctor
    available_dates = fetch_available_dates_for_whatsapp(doctor_id)

    url = f"https://graph.facebook.com/v16.0/{phone_number_id}/messages"
    headers = {"Authorization": f"Bearer {GRAPH_API_TOKEN}"}

    # Prepare sections for the list of available dates
    sections = [{
        "title": get_translated_text("Available Dates", user_sessions[sender]["language"]),
        "rows": [{"id": date, "title": date} for date in available_dates]
    }]
...
def send_time_list(sender):
    # Retrieve the selected doctor and date from the user session
    doctor_id = user_sessions[sender]["doctor"]
    selected_date = user_sessions[sender]["selected_date"]

    # Fetch available times for the selected doctor and date
    available_times = fetch_available_times_for_whatsapp(doctor_id, selected_date)

    url = f"https://graph.facebook.com/v16.0/{phone_number_id}/messages"
    headers = {"Authorization": f"Bearer {GRAPH_API_TOKEN}"}

    # Prepare sections for the list of available times
    sections = [{
        "title": get_translated_text("Available Times", user_sessions[sender]["language"]),
        "rows": [{"id": time, "title": time} for time in available_times]
    }]
...
def fetch_available_dates_for_whatsapp(doctor_id):
    conn = get_db_connection()
    cursor = conn.cursor()
...
def fetch_available_times_for_whatsapp(doctor_id, selected_date):
    conn = get_db_connection()
    cursor = conn.cursor()
